...
main (int argc, char *argv[])
{
  ...
  mongoc_query_flags_t mflags;
  cursor = mongoc_collection_find (collection, mflags, 0, 1000, 5, query, NULL, NULL);
  ...
  mongoc_insert_flags_t miflags;
  if (!mongoc_collection_insert (collection, miflags, &userconf, NULL, &error)) {
    fprintf (stderr, "%s\n", error.message);
  }
